...
class TimezoneTypeTransformerTest extends BaseTransformerTest
{
    protected static string $formType = TimezoneType::class;

    protected static string $type = 'string';

    protected static string $widget = 'timezone';
    ...
    public function testOverriddenOptions()
    {
        $this->common(self::$formType, self::$property, self::$type, self::$widget, null, [
            'expanded' => false,
            'multiple' => false,
            'preferred_choices' => $preferredChoices = ['UTC', 'Europe/Paris'],
        ]);
